...
# Folded in from the former Spatium_PrincipalExtract.py. The 6x6 elasticity
# tensor C_ij is the COMPLETE anisotropy characterisation of an RVE.
#   abaqus python Spatium_PostProcess.py principal   <odb> <out> [L] [run_id]
#   abaqus python Spatium_PostProcess.py elasticity  <odb_dir> <out> <L> <run_id>
# =====================================================================
def _openOdb(*a, **k):
    from odbAccess import openOdb
    return openOdb(*a, **k)
...
# tool so the pipeline needs ONLY Spatium_Standalone / Spatium_GmshPeriodic /
# Spatium_PostProcess. Invoke:  python Spatium_PostProcess.py analyze <name> ...
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == 'principal':
        if len(sys.argv) < 4:
            print("Usage: abaqus python Spatium_PostProcess.py principal <odb> <out> [L] [run_id]")
            sys.exit(1)
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == 'elasticity':
        if len(sys.argv) < 6:
            print("Usage: abaqus python Spatium_PostProcess.py elasticity <odb_dir> <out> <L> <run_id>")
            sys.exit(1)
    ...
    if len(sys.argv) < 3:
        ...
        print("       python Spatium_PostProcess.py analyze <eq19|lengthscale|homog-calib|hybrid|rve-study> ...")
        sys.exit(1)
