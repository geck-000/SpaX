...
def main():
    ...
    with open(p, 'w', newline='', encoding='utf8') as fh:
        w = csv.writer(fh)
        w.writerow(COLS)
        for L in (0.24, 0.32, 0.40, 0.48):
            n_sl = int(round(L / 0.08))
            ...
            for s in (1, 2, 3, 4, 5):
                ...
                n += 1
    print('wrote %s  (%d cells)' % (p, n))
    print('   a_0 held at 0.080, bridge density at 32/unit area, full tensor on')
    for L in (0.24, 0.32, 0.40, 0.48):
        n_sl = int(round(L / 0.08)); n_br = max(1, int(round(32.0 * L * L)))
