...
# elasticity_tensor_<run_id>.csv per slice into post_coltensor/. L=0.50.
unset SLURM_GTIDS
...
for z in 05 15 25 35 45 55 65 75 85 95; do
    ...
    abaqus python SpaX_PostProcess.py elasticity "$WORKDIR" post_coltensor 0.50 "$RID"
done

echo "===== tensors written ====="
ls -1 post_coltensor/elasticity_tensor_CTEN_z*.csv 2>/dev/null
