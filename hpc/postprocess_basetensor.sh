...
cd "$WORKDIR" || exit 1
mkdir -p tensors/basetensor_seeds
...
for s in 1 2 3 4 5; do
    ...
    abaqus python SpaX_PostProcess.py elasticity "$WORKDIR" tensors/basetensor_seeds 0.50 "$RID"
done

echo "===== tensors written ====="
ls -1 tensors/basetensor_seeds/elasticity_tensor_BTEN_z95_s*.csv 2>/dev/null
