...
F += [
 ...
 ('c: lower end (kujala)',  max(_c_kujala[0], _c_gogo[0]),           0.470),
 ('c: upper end (gogolaze)', min(_c_kujala[1], _c_gogo[1]),          0.575),
 ('c: gogolaze lower',      _c_gogo[0],                              0.318),
 ('c: kujala upper',        _c_kujala[1],                            0.601),
 ('sens: exponent +-2rms',  _E(n=_n - 2 * _ez.N_FIT_RMS) / _E(n=_n + 2 * _ez.N_FIT_RMS), 1.19),
 ('sens: a0 0.5-1.5 mm',    _E(a0_mm=1.5) / _E(a0_mm=0.5),           2.08),
 ('sens: phi_0 0.15-0.36',  _E(phi_0=0.36) / _E(phi_0=0.15),         4.48),
 ('sens: phi_0 0.14-0.23',  _E(phi_0=0.23) / _E(phi_0=0.14),         4.51),
 ...
 ('plate: E_flex/E_ext',    _plate['Eflex'] / _plate['Eext'],        0.851),
 ('plate: E^t/E_flex',      _plate['Et'] / _plate['Eflex'],          1.208),
 ('plate: 12D about mid',   _plate['D12mid'],                        7.653),
 ('gogolaze rigidity, step', _gogo('step'),                          2.470),
 ('gogolaze rigidity, ramp', _gogo('ramp'),                          3.317),
...
STALE = [
 ...
 # 'E_{\\mathrm{flex}}=1.22' was a stale marker for the pre-n(b) flexural
 # ratio.  The promoted pocket law returns E^t/E_flex = 1.222, so 1.22 is
 # now the CORRECT value and the guard would fire on it; the claims table
 # above pins the quantity directly instead.
