...
\begin{itemize}[nosep]
  ...
  \item Mesh resolution: porous $D_\text{rve}$ is sensitive to inclusion
        resolution (a controlled same-seed test shows $+30\%$ when coarsening
        $\code{L\_mesh}\,0.033\!\to\!0.045$), so coarsening is \emph{not} a safe
        speed-up; the matrix-side homogeneous response is mesh-insensitive.
