...
\begin{itemize}[nosep]
  \item \code{SpaX\_Standalone.py} --- packing, periodic meshing driver,
        Abaqus input-deck (\code{.inp}) generation;
  \item \code{SpaX\_GmshPeriodic.py} --- the periodic Gmsh mesher;
  \item \code{SpaX\_PostProcess.py} --- ODB extraction, effective properties,
...
transverse isotropy of Sec.~\ref{sec:aniso}. For the realistic channelled-base
morphology a three-size quadratic bending sweep ($L=0.24,0.32,0.40$; $L/d=3,4,5$;
four seeds each) gives a negative fit slope ($-4.4\times10^{7}$) --- an imaginary
$\ell$ --- with the ratio $E_\text{bend}^\text{mat}/E^\ast_\text{1st}$ rising
$0.92\!\to\!0.96\!\to\!1.00$ as $L$ grows: the weak-inclusion softening simply
decays with size, leaving no size-independent excess and confirming the null.

%==============================================================
\section{Consolidated tool and command-line interface}
\label{sec:cli}
All former standalone analysis scripts are folded into
\code{SpaX\_PostProcess.py}. \code{odbAccess} is imported lazily, so the
...
\bottomrule
\end{tabular}
\caption{Environment variables controlling the new features (all opt-in;
defaults reproduce the previous pipeline).}
\label{tab:env}
\end{table}
...
\begin{itemize}[nosep]
  ...
  \item Eq.~19 reproduces the standalone analyzer exactly and, across all
        ...
        microstructure --- including a full three-size sweep ($L=0.24,0.32,0.40$)
        of the channelled-base morphology (slope $-4.4\times10^{7}$, ratio
        $\to1.0$), so the second-order result is a clean size-confirmed null.
\end{itemize}
